	...
	DirectiveHandler.prototype = {
		...
		handleDirective: function(directive, directiveName, currentFile, variables) {
			...
			switch (directiveName) {
				...
				case "config":
					interpolate.apply(this);
					return this._handleConfig(attributes, currentFile, variables);
			}
		...
		_handleEcho: function(attributes, variables) {
			...
			if (attributeVal === 'DATE_LOCAL') {
				var now = new Date();
				
				return {output: dateFormat(now, this.this._config.dateFormat)};
			}
			else if (attributes.length == 1 && attributes[0].name === "var") {
		...
		%Y%m%d%H%M%S
		*/
		_handleConfig: function(attributes, currentFile, variables) {
			var attribute = attributes[0];
			var attributeName = attribute.name;
			var fmt = attribute.value;
			var results = {output: ''};
			
			var dateFmt = fmt.replace(/%y/i,'yyyy').replace('%m', 'mm').replace(/%d/i,'dd');
			dateFmt = dateFmt.replace(/%h/i,'hh').replace('%M','MM').replace(/%s/i,'ss');
			
			switch(attributeName) {
				case 'timefmt': this._config.dateFormat = dateFmt;
			}
